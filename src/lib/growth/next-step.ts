...
export type CoordinatedNextStep = {
  ...
  needsDraftBrandVoice: boolean
  needsSaveReviewSchedule: boolean
  needsRunSeo: boolean
...
export function coordinateNextStep(input: NextStepInput): CoordinatedNextStep {
  ...
  return {
    ...
    needsAddOffer: input.reports.some(
      (report) => report.recommend.title === ADD_OFFER_STEP_TITLE,
    ),
    ...
    needsDraftBrandVoice: input.reports.some(
      (report) => report.recommend.title === DRAFT_BRAND_VOICE_STEP_TITLE,
    ),
    needsSaveReviewSchedule: input.reports.some(
      (report) => report.recommend.title === SAVE_REVIEW_SCHEDULE_STEP_TITLE,
    ),
    needsRunSeo: input.reports.some(
      (report) => report.recommend.title === RUN_SEO_STEP_TITLE,
    ),
    needsSeoDraft: input.reports.some(
      (report) =>
        report.recommend.title === FIX_SEO_STEP_TITLE ||
        report.recommend.title === IMPROVE_SEO_STEP_TITLE,
    ),
    executeAllowed: false,
  };
}
