...
export const businessBrains = pgTable("business_brains", {
  ...
  notes: text("notes").notNull().default(""),
  idealCustomers: jsonb("ideal_customers").$type<string[]>().notNull().default([]),
...
export const offers = pgTable(
  "offers",
  {
    ...
    discoveryStatus: text("discovery_status").notNull().default("confirmed"),
    inferredFrom: text("inferred_from").notNull().default(""),
    ...timestamps,
  },
...
export const seoProposalPacks = pgTable(
  "seo_proposal_packs",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    organizationId: uuid("organization_id")
  ...
  (table) => [index("seo_proposal_packs_org_idx").on(table.organizationId)],
);

export const seoProposalItems = pgTable(
  "seo_proposal_items",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    organizationId: uuid("organization_id")
    ...
    packId: uuid("pack_id")
      .notNull()
      .references(() => seoProposalPacks.id, { onDelete: "cascade" }),
    externalId: text("external_id").notNull(),
    type: text("type").notNull(),
  ...
  (table) => [
    index("seo_proposal_items_org_idx").on(table.organizationId),
    index("seo_proposal_items_pack_idx").on(table.packId),
  ],
);
