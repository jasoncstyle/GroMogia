...
**Phase 7 — Website builder**, full-page editor with preview (not live), numbered starter layouts (white with dark grey text), a color wheel plus hex and swatches, extra GroovGro pages, and SEO checks for the connected homepage plus every GroovGro page. Publish per page. Home cannot be deleted. Does not replace a connected existing website. Does not change Stripe checkout. Public groovgro.com homepage stays Coming soon.
...
- Website builder: visual GroovGro-hosted pages at `/w/[org]` (Home) and `/w/[org]/[slug]` (extra pages), with a labeled starting template, full-page editor, draft preview, per-row width, white/dark-grey defaults, a color wheel plus hex and swatches, extra widgets, and photos fetched by GroovGro
- Approved SEO title, description, and heading drafts can be applied to the matching GroovGro page

## Still later

Builder custom domains, image uploads (Vercel Blob), nested inner rows, WordPress write adapter, Phase 8 integrations (including Google Ads), 9 AI execute, 10 commercialization.
