...
- Search-to-page loop (first slice): one stored Search Console query, tied to a matching offer and the active Goal when those exist. Worth-a-look queries come first. If none are marked worth a look, GroovGro still picks a stored query and the owner can use another stored search. There is no box to type a Google search. When no query is stored, the card lists the steps and offers Refresh Search Console and Read the website. The owner saves a brief, writes a workspace draft from saved brand, offer, difference, and brand-voice facts, pastes it on the existing site, then checks stored Search Console numbers and the Goal. SEO is a quiet search desk; other SEO tools sit behind More SEO tools. A hashed desk token lets owner-created Grok bots read the stored snapshot only. The owner can copy that stored Search Console into SEO Scout and paste a JSON proposal pack back. Pack items stay proposals until the owner approves. Only GroovGro marks shipped, after a real apply. There is no live GroovGro↔Scout API. GroovGro does not invent prices, call a model, publish, scrape Google, or overwrite the live website. Scout does not log into Google.
