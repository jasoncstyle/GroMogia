...
**Status:** vision approved 10 September 2026. Partner model recorded 11 September 2026. Phases A–T first slices are implemented. Owner-named competitor looks and owner-run suggested searches are implemented. Do not turn on execute, Growth Director, SERP lookup, competitor search, GEO lookup, or CMS publish.
...
- Competitor and SERP lookup (contracted, allowed providers only). Owner-entered notes, owner-named website looks, and owner-run suggested searches exist. Automated finding from search terms stays behind an off adapter. Do not scrape Google.
