...
**Status:** vision approved 10 September 2026. Phases A–E first slices are implemented. Do not start Phase F–T until Jason asks.
...
- Keyword model and history from those stored Search Console queries (no vendor, no score)
...
- Keywords: Search Console queries are stored as a keyword model with snapshot history (Phase E). Groups, intent, opportunity scores, and paid keyword vendors stay **PLANNED**.
...
- Keyword groups, intent, opportunity scores, create-vs-improve, paid keyword vendors
...
Today: title, description, heading, and similar page checks; owner-approved drafts; Search Console totals and top queries; those existing sources can create recommend-only Growth Actions when evidence clears conservative thresholds; stored Search Console queries become a keyword model with snapshot history.

Future: keyword groups; intent; opportunity scoring; SERP and competitor analysis; content gaps; page improvement; create vs improve; conversion- and revenue-aware priority.
