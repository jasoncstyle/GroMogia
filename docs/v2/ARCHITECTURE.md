...
370. **Bot team roster** — **IMPLEMENTED** this slice. Names are **SEOgro**, **DRAFTgro**, and **BOOKSgro**. SEOgro is live. DRAFTgro and BOOKSgro handoffs are later and do not block the SEO loop. GET `/api/bots/scout` returns `team`, `gsc`, and `publicPages`. See [BOT_TEAM.md](BOT_TEAM.md).
