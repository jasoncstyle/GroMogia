...
371. **Business switcher** — **IMPLEMENTED** this slice. The sidebar (and phone menu) lists organizations the signed-in user belongs to. Picking one opens that whole workspace and stays on the same app page. The last business is remembered in a cookie. GroovGro does not mix brands on one screen. Desk tokens stay per organization.
