...
166. **Phases O–T** — **PLANNED** in [MASTER_BRIEF.md](../MASTER_BRIEF.md) §16: GEO audits, CMS adapters, attribution, experiments, gated execute. Do not rebuild V2 to fit the letters. Do not turn on SERP lookup or GEO lookup.
...
- GEO facts in this slice are owner-entered `geo_notes`, `geo_queries`, and `geo_history`. `requestGeoLookup` stays off. Do not hard-code a vendor list. Do not scrape AI answers. Do not treat one AI answer as truth.
