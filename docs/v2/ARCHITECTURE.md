...
370. **Bot team roster** — **IMPLEMENTED** this slice. Names are **SEOgro**, **DRAFTgro**, **WRITEgro**, and **BOOKSgro**. All four have live handoffs. dr eggbot stays out of the data loop. See [BOT_TEAM.md](BOT_TEAM.md).
...
372. **DRAFTgro, WRITEgro, and BOOKSgro handoffs** — **IMPLEMENTED** this slice. Same desk token as SEOgro. `GET/POST /api/bots/draft`, `/write`, and `/books`. Items land as proposed. DRAFTgro does not send. WRITEgro does not publish. BOOKSgro reads stored payment copies only and does not move money. Monday review is on Bot team.
