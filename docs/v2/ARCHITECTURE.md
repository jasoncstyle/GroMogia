...
307. **Channel-compare heading shows how many are worth a look** — **IMPLEMENTED** this slice. The stored-evidence compare heading on SEO says how many channel compares are worth a look. GroovGro does not change Next step or add a Next step loop.
308. **Channel-compare copy names worth-a-look channels listed first** — **IMPLEMENTED** this slice. The stored-evidence compare card says channels worth a look are listed first. GroovGro does not change Next step or add a Next step loop.
...
317. **Competitor-notes heading shows remaining known names without a note** — **IMPLEMENTED** this slice. The competitor-notes heading on SEO says how many Business-known competitors still need a note. GroovGro does not scrape search results or add a Next step loop.
...
344. **Library questions that still need a why are listed first** — **IMPLEMENTED** this slice. The library-questions list shows questions that still need a why first. GroovGro does not ask an AI system or add a Next step loop.
345. **Library-questions copy names questions that still need a why listed first** — **IMPLEMENTED** this slice. The library-questions card says questions that still need a why are listed first. GroovGro does not ask an AI system or add a Next step loop.
346. **Intelligence names questions that still need a why listed first** — **IMPLEMENTED** this slice. Intelligence can point at saved library questions and say questions that still need a why are listed first. GroovGro does not ask an AI system or add a Next step loop.
347. **Library-questions heading shows how many still need a why** — **IMPLEMENTED** this slice. The library-questions heading says how many saved questions still need a why. GroovGro does not ask an AI system or add a Next step loop.
348. **SEO intro names questions that still need a why listed first** — **IMPLEMENTED** this slice. The SEO page says library questions that still need a why are listed first. GroovGro does not ask an AI system or add a Next step loop.
...
365. **Quiet search desk** — **IMPLEMENTED** this slice. SEO shows the Goal, one search-to-page next step, a visible trail, and Search Console. Other SEO tools sit under More SEO tools. GroovGro does not scrape Google or publish.
366. **Stored desk for Search partner and Goal checker** — **IMPLEMENTED** this slice. GroovGro pulls Search Console and stores it. A hashed desk token on Integrations lets owner-created bots read that store at `/api/bots/search-desk`. Bots do not log into Google. GroovGro does not publish or invent prices.
367. **SEOgro proposal inbox** — **IMPLEMENTED** this slice. GroovGro receives SEOgro proposal packs as proposed. The owner reviews on Monday (approve / reject). Only GroovGro marks shipped after a real apply. Paste is a sample of the live handoff, not the courier product. SEOgro does not log into Google or publish. Property is a label on the pack, not a hard-coded business.
368. **SEOgro live handoff** — **IMPLEMENTED** this slice. `/api/bots/scout` lets SEOgro GET stored Search Console plus public URL inventory and POST a proposal pack with the desk token. Items land as proposed. The product is GroovGro talking to SEOgro. The owner is the Monday reviewer, not the paste courier. SEOgro does not log into Google, publish, or set shipped. Live CMS apply stays off.
369. **Visual owner dashboard** — **IMPLEMENTED** this slice. Dashboard shows stored Goal progress, leads, payments, named-share visits, Search Console queries GroovGro already pulled, and SEOgro proposals as a visual desk. It does not invent shop metrics or scrape Google.
370. **Bot team roster** — **IMPLEMENTED** this slice. Names are **SEOgro**, **DRAFTgro**, **WRITEgro**, and **BOOKSgro**. SEOgro, DRAFTgro, and WRITEgro are live. BOOKSgro is later. dr eggbot stays out of the data loop. See [BOT_TEAM.md](BOT_TEAM.md).
371. **Business switcher** — **IMPLEMENTED** this slice. The sidebar (and phone menu) always shows the open business. The owner can switch organizations or add another one. Adding a business creates a separate organization, membership, and optional website URL, then opens that workspace. The last business is remembered in a cookie. GroovGro does not mix brands on one screen. Desk tokens stay per organization. Do not hard-code business names.
372. **DRAFTgro and WRITEgro handoffs** — **IMPLEMENTED** this slice. Same desk token as SEOgro. `GET/POST /api/bots/draft` and `/write`. Items land as proposed. DRAFTgro does not send. WRITEgro does not publish. BOOKSgro is parked: `/api/bots/books` refuses packs and does not send payment copies. Monday review is on Bot team.
373. **GA4 read-only connect** — **IMPLEMENTED** this slice. `/api/google-analytics/start` asks for `analytics.readonly` only. GroovGro lists GA4 properties, stores a 28-day snapshot (sessions, landing pages, sources), and shows it on Analytics, Search desk, and Integrations. Ads stay off. SEOgro does not read this snapshot yet. Owner still must enable Analytics APIs and the callback URI in Google Cloud before Connect works. See [../phase-6/USER_SETUP.md](../phase-6/USER_SETUP.md) section G.
374. **SEOgro keyword history on the handoff** — **IMPLEMENTED** this slice. `GET /api/bots/scout` includes stored Search Console keyword history and worth-a-look labels. Estimates from GroovGro’s store only. Not Keyword Planner. Not OpenSERP. Do not scrape Google.
375. **Shared refresh scheduler** — **IMPLEMENTED** this slice. A `scheduled_jobs` registry plus Vercel Cron (`/api/cron/jobs`, declared in `vercel.ts`) can refresh Search Console, GA4, public website pages, and Stripe copies per organization. Frequencies: manual, daily, weekly. Owner refresh buttons stay. Cron does not publish, send, buy ads, or wake bots. New read-only refreshes register in `src/lib/jobs/catalog.ts`. Remaining work after A–T first slices: [../REMAINING.md](../REMAINING.md).
