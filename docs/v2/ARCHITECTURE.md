...
367. **SEOgro proposal inbox** — **IMPLEMENTED** this slice. The owner copies stored Search Console into SEOgro and pastes a JSON proposal pack back. Items stay proposed until the owner approves. Only GroovGro marks shipped after the owner applies the change. SEOgro does not log into Google or publish. Property is a label on the pack, not a hard-coded business.
368. **SEOgro live handoff** — **IMPLEMENTED** this slice. `/api/bots/scout` lets SEOgro GET stored Search Console and POST a proposal pack with the desk token. Items land as proposed. Paste is a sample of the same contract, not the courier product. SEOgro does not log into Google, publish, or set shipped. Live CMS apply stays off.
