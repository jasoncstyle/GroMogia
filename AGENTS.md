...
- Competitor looks from owner-named URLs are implemented: the owner can save a competitor website on SEO. GroovGro may fetch that homepage and a few public pages on the same site, then store how they sell, how they market, and how we might compete. If the host blocks that request, GroovGro may read the same owner-named URL through a public page reader, or the owner can paste the page. The owner can run a suggested search themselves and save a site they found. SEO can compare those stored looks to what this business sells, and name topics those sites show that GroovGro has not read on this business’s pages. The owner can save what they will do, including from a competitor page topic, and mark that move done. `requestCompetitorSearch` / `competitorSearchEnabled()` stay off. Do not scrape Google, Bing, or social networks. Do not copy competitor words onto a live site. Do not create a page from that look. Do not do that owner-saved work.
...
- Phase I first slice is implemented: the owner can save a content brief to the SEO planner, including from a competitor page-topic gap. Do not publish. Do not copy competitor words.
- Phase J first slice is implemented: GroovGro can write a workspace draft from a saved brief. A draft from a competitor-topic brief uses this business’s words and saved offers. GroovGro can check whether that draft names a saved offer or what makes this business different. Do not publish, copy competitor words, or change the live website.
...
- Phase P first slice is implemented: the owner can save a workspace draft for later CMS review, including from the Content planner next to that draft, with an optional note about where they already publish. The adapter stays off. Do not publish, write WordPress or Shopify, overwrite a connected live site, or resume the paused website builder.
